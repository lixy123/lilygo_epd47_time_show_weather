...
class weatherManager {
  private:
    ...
    String  calcWeekOfDay_string(String date_str);
    int calcWeekOfDay(int Years, int Months, int Days);
    bool isLeapYear(int Years);
    uint8_t * get_icon(String no);
  public:
    weatherManager();
    ~weatherManager();
    void draw_weather(String weather_result, String memo1,String memo2);
